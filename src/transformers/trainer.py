...
from .adapters.lora import Linear, LoRALayer
...
class Trainer:
    ...
    def __init__(
        ...
        self.add_callback(CustomEvalCallback)
    ...
    def _inner_training_loop(
        ...
        # a hack way to solve the device mismatch problem when resuming from checkpoint
        for name, layer in model.named_modules():
            for param in layer.parameters():
                if param.data.device != torch.device("cuda:0"):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                for name, layer in model.named_modules():
                    if isinstance(layer, Linear) or isinstance(layer, AdapterLayer):
                        if hasattr(layer, "adapter_name") and layer.adapter_name != '':
                            # apply_masks_to_grads_of_tied_weights provides correct grads for each masks
                            layer.apply_masks_to_grads_of_tied_weights(layer.adapter_name)
                            # gather all grads and sum up
                            layer.aggregate_grads_for_tied_weights(layer.adapter_name)
                            #restore zero elements and ready for gradient descent
                            layer.restore_dense_tied_weights(layer.adapter_name)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    for name, layer in model.named_modules():
                        if isinstance(layer, Linear) or isinstance(layer, AdapterLayer):
                            if hasattr(layer, "adapter_name") and layer.adapter_name != '':
                                #make sure all tied-layers have same values
                                layer.get_latest_dense_tied_weights(layer.adapter_name)
                                #make sure masks correctly pruning each tied-layers
                                layer.apply_masks_to_dense_tied_weights(layer.adapter_name)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_test and self.predict_dataset is not None:
            test_metrics = self.evaluate(eval_dataset=self.predict_dataset, metric_key_prefix="test")
            self._report_to_hp_search(trial, self.state.global_step, test_metrics)
